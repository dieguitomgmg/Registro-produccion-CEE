...
if st.session_state.etapa == "linea":
    ...
    if st.session_state.mostrar_edicion:
        ...
        registro_str = st.selectbox("Selecciona un registro", list(opciones_dict.keys()))
        ...
        if registro["Operario"] not in ["Inicio producción", "Parada producción"]:
            nuevo_operario = st.text_input("👤 Operario", value=registro["Operario"])
            nuevo_turno = st.radio("🕒 Turno", ["Mañana", "Tarde", "Noche"], index=["Mañana", "Tarde", "Noche"].index(registro["Turno"]))
            nuevo_producto = st.selectbox("🧱 Producto", ["Producto A", "Producto B", "Producto C"], index=["Producto A", "Producto B", "Producto C"].index(registro["Producto"]))
            nuevas_producidas = st.number_input("🔢 Unidades producidas", min_value=0, value=int(registro["Unidades producidas"]), step=1)
            nuevas_defectuosas = st.number_input("❌ Unidades defectuosas", min_value=0, value=int(registro["Unidades defectuosas"]), step=1)
            nuevas_observaciones = st.text_area("📝 Observaciones", value=registro["Observaciones"])
