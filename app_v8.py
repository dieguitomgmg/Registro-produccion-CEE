...
if st.session_state.etapa == "CEE":
    ...
    if "csv_path" in st.session_state and os.path.exists(st.session_state.csv_path):
        ...
        # --- Sección para editar o eliminar registros recientes ---
    st.markdown("### ✏️ Modificar o eliminar un registro reciente")

    # Mostrar los últimos 5 registros con un índice
    ultimos_registros = st.session_state.df.tail(5).reset_index(drop=True)
    registro_seleccionado = st.selectbox("Selecciona un registro para modificar o eliminar", ultimos_registros.index, format_func=lambda i: f"{ultimos_registros.loc[i, 'Fecha']} - {ultimos_registros.loc[i, 'Operario']}")

    registro = ultimos_registros.loc[registro_seleccionado]

    # Mostrar formulario para editar campos
    ...
    nuevo_turno = st.radio("🕒 Turno", ["Mañana", "Tarde", "Noche"],
                       index=["Mañana", "Tarde", "Noche"].index(registro["Turno"]),
                       key="editar_turno")
    nuevo_producto = st.selectbox("🧱 Producto", ["Producto A", "Producto B", "Producto C"], index=["Producto A", "Producto B", "Producto C"].index(registro["Producto"]), key="editar_producto")
    nuevas_producidas = st.number_input("🔢 Unidades producidas", min_value=0, value=int(registro["Unidades producidas"]), step=1, key="editar_producidas")
    nuevas_defectuosas = st.number_input("❌ Unidades defectuosas", min_value=0, value=int(registro["Unidades defectuosas"]), step=1, key="editar_defectuosas")
