# -*- coding: utf-8 -*-
"""FAR Panel registro datos_v8 entrenamiento
...
if st.session_state.etapa == "CEE":
    ...
    # Si se pulsa este botón, activamos la bandera para solicitar la causa.
    if st.button("⛔ Parada por incidencia", key="btn_parar_prod"): # Añadí key
        st.session_state.solicitar_causa_parada = True
        st.session_state.produccion_iniciada = False # La producción se detiene al pulsar esto

    # --- Lógica para solicitar la causa de la parada ---
    if st.session_state.solicitar_causa_parada:
        st.markdown("### 🛑 Registrar Parada por Incidencia")
        causa_parada = st.text_area("Explica la causa de la parada (requerido):", value=st.session_state.causa_parada_temp, key="input_causa_parada")
        st.session_state.causa_parada_temp = causa_parada # Guardar temporalmente para persistir si se recarga

        if st.button("✅ Confirmar Parada", key="btn_confirmar_parada"):
            if not causa_parada.strip(): # Validar que no esté vacío
                st.error("Por favor, describe la causa de la parada.")
            else:
                zona = ZoneInfo("Europe/Madrid")
                fila_parada = {
                    "Fecha": datetime.now(zona).strftime("%Y-%m-%d"),
                    ...
                    "Unidades producidas": "",
                    "Unidades defectuosas": "",
                    "Observaciones": f"Parada de la producción por incidencia: {causa_parada}"
                }
                ...
                st.session_state.solicitar_causa_parada = False # Ocultar el formulario de causa
                st.session_state.causa_parada_temp = "" # Limpiar el campo
                st.error("⛔ Parada registrada con la causa especificada.")
                st.rerun()
        if st.button("🚫 Cancelar Parada", key="btn_cancelar_parada"):
            st.session_state.solicitar_causa_parada = False
            st.session_state.produccion_iniciada = st.session_state.produccion_iniciada_antes_parada if "produccion_iniciada_antes_parada" in st.session_state else False
            st.session_state.causa_parada_temp = ""
            st.rerun()
