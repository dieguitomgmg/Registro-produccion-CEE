...
def eliminar_ultimo_registro_y_revertir_estado():
    ...
    ultimo = df.iloc[-1]
    tipo_ultimo = ultimo["Tipo de operación"]

    # Operaciones que modifican el estado del sistema
    operaciones_estado = ["Inicio de producción", "Parada por incidencia"]

    # Eliminar último registro
    df = df.iloc[:-1]
    df.to_csv(st.session_state.csv_path, index=False, sep=';', encoding='utf-8-sig')
    st.success("🗑️ Último registro eliminado correctamente.")

    # Si el tipo de operación alteraba el estado, revertir al anterior
    if tipo_ultimo in operaciones_estado and not df.empty:
        penultimo = df.iloc[-1]
        tipo_penultimo = penultimo["Tipo de operación"]

        # Lógica para determinar el estado anterior en función del tipo
        ...
        if st.button("✅ Registrar reanudación"):
            if motivo_reanudacion.strip() == "":
                ...
                fila_reanudacion = {
                    ...
                    "Línea": st.session_state.linea,
                    "Estación": st.session_state.estacion,
                    "Operario": "Reanudación producción",
                    "Turno": "",
                    "Producto": "",
                    "Unidades producidas": "",
                    "Unidades defectuosas": "",
...
if st.session_state.etapa == "linea":
    ...
    if st.session_state.mostrar_edicion:
        ...
        if st.button("🗑️ Eliminar registro seleccionado", key=f"eliminar_{clave_registro}"):
            eliminar_ultimo_registro_y_revertir_estado()
