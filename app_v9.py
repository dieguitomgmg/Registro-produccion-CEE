...
def eliminar_registro_por_indice(indice):
    df = pd.read_csv(st.session_state.csv_path, sep=';', encoding='utf-8-sig')

    if df.empty or indice not in df.index:
        st.warning("No se puede eliminar: el índice no existe.")
        return

    # Verificar si es el último registro
    es_ultimo = indice == df.index[-1]

    # Verificar si el tipo de operación modifica el estado
    registro_a_eliminar = df.loc[indice]
    tipo_operacion = registro_a_eliminar.get("Estado", "")

    operaciones_estado = ["Reanudación producción", "Parada producción"]

    # Eliminar el registro
    df = df.drop(index=indice).reset_index(drop=True)
    df.to_csv(st.session_state.csv_path, index=False, sep=';', encoding='utf-8-sig')
    st.success("🗑️ Registro eliminado correctamente.")

    # Actualizar DataFrame en sesión
    st.session_state.df = df

    # Si era el último y modifica el estado, revertir al estado anterior
    if es_ultimo and tipo_operacion in operaciones_estado:
        if not df.empty:
            penultimo = df.iloc[-1]
            tipo_penultimo = penultimo.get("Tipo de operación", "")

            if tipo_penultimo == "Inicio de producción":
                st.session_state.estado_actual = "En producción"
            elif tipo_penultimo == "Parada por incidencia":
                st.session_state.estado_actual = "En parada"
            else:
                st.session_state.estado_actual = "Sin producción"
        else:
            st.session_state.estado_actual = "Sin producción"
...
if st.session_state.etapa == "CEE":
    ...
    parada_disabled = st.session_state.estado_linea == "Parada por incidencia"
...
if st.session_state.etapa == "linea":
    ...
    if st.session_state.mostrar_edicion:
        ...
        if registro["Estado"] not in ["Reanudación producción", "Parada producción"]:
            ...
            nuevo_producto = st.selectbox("🧱 Producto", ["Producto A", "Producto B", "Producto C"], index=["Producto A", "Producto B", "Producto C"].index(registro["Producto"]), key=f"producto_{clave_registro}")
            nuevas_producidas = st.number_input("🔢 Unidades producidas", min_value=0, value=int(registro["Unidades producidas"]), step=1)
            nuevas_defectuosas = st.number_input("❌ Unidades defectuosas", min_value=0, value=int(registro["Unidades defectuosas"]), step=1, key=f"defectuosas_{clave_registro}")
