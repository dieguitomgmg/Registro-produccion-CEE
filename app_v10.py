...
if modo == "Iniciar sesión":
    ...
    if st.button("Entrar"):
        if usuario in usuarios and usuarios[usuario]["clave"] == clave:
            ...
            st.session_state.etapa = "inicio"  # ✅ Esto es lo que hace avanzar
            st.rerun()
        else:
...
if st.session_state.usuario_logueado:
    ...
    st.markdown("### ➕ Añadir línea de producción")
    nueva_linea = st.text_input("Nombre de la línea")
    num_estaciones = st.number_input("Número de estaciones", min_value=1, step=1)
    nombres_estaciones = []
    for i in range(int(num_estaciones)):
        nombre = st.text_input(f"🧱 Nombre de la estación {i+1}", key=f"estacion_{i+1}")
        nombres_estaciones.append(nombre)

    if st.button("Agregar línea"):
        if nueva_linea in usuarios[usuario]["lineas"]:
            st.warning("⚠️ Esta línea ya existe")
        else:
    ...
    for linea, estaciones in usuarios[usuario]["lineas"].items():
        st.write(f"🔹 {linea}: {len(estaciones)} estaciones")
        for idx, nombre_est in enumerate(estaciones, 1):
            st.write(f"   - Estación {idx}: {nombre_est}")

    # Luego puedes permitir iniciar el registro de producción aquí...
