...
def eliminar_registro_por_indice(indice):
    ...
    if es_ultimo and estado_actual in ["Parada producción", "Reanudación producción"]:
        ...
        df_antes = df[df.index < indice]

        if not estacion or estacion == "":
            estaciones_linea = usuarios[usuario]["lineas"][linea]["nombres"]
            estaciones_actualizadas = set()

            for i in reversed(df_antes.index):
                fila = df_antes.loc[i]
                if fila["Línea"] != linea:
                    continue

                est = str(fila["Estación"]).strip()
                estado = fila["Estado"]

                if est == "":
                    nuevo_estado = (
                        "En producción" if estado == "Reanudación producción"
                        else "Parada por incidencia" if estado == "Parada producción"
                        else "En producción"
                    )
                    for est2 in estaciones_linea:
                        est2 = str(est2).strip()
                        if est2 not in estaciones_actualizadas:
                            key = f"estado_linea_{linea}_Estacion_{est2}"
                            st.session_state[key] = nuevo_estado
                            estaciones_actualizadas.add(est2)
                    break

                else:
                    if est in estaciones_actualizadas:
                        continue
                    nuevo_estado = (
                        "En producción" if estado == "Reanudación producción"
                        else "Parada por incidencia" if estado == "Parada producción"
                    ...
                    key = f"estado_linea_{linea}_Estacion_{est}"
                    st.session_state[key] = nuevo_estado
                    estaciones_actualizadas.add(est)

            for est2 in estaciones_linea:
                est2 = str(est2).strip()
                if est2 not in estaciones_actualizadas:
                    key = f"estado_linea_{linea}_Estacion_{est2}"
                    st.session_state[key] = "En producción"

        else:
            key_estado = f"estado_linea_{linea}_Estacion_{estacion}"
            estado_estacion = "En producción"

            df_estacion_antes = df_antes[
                (df_antes["Línea"] == linea) &
                ((df_antes["Estación"] == "") | (df_antes["Estación"] == estacion))
            ]

            if not df_estacion_antes.empty:
                estado_anterior = df_estacion_antes.iloc[-1]["Estado"]
                if estado_anterior == "Reanudación producción":
                    estado_estacion = "En producción"
                elif estado_anterior == "Parada producción":
                    estado_estacion = "Parada por incidencia"

            st.session_state[key_estado] = estado_estacion

        guardar_estados()
        estado_linea = st.session_state.get(f"estado_linea_{linea}", "No iniciada")
        st.session_state.produccion_iniciada = (estado_linea == "En producción")
