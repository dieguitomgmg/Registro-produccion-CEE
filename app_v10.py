...
if st.session_state.etapa == "linea":
    ...
    with col1:
        if st.button("📊 Ir a producción"):
            ...
            st.session_state.etapa = "seleccion_linea"
            st.rerun()
    with col2:
        if st.button("⚙️ Configurar líneas y estaciones"):
    ...
    for idx, (nombre_linea, info) in enumerate(datos.items()):
        st.subheader(f"✏️ Línea {idx + 1}")
        nuevo_nombre_linea = st.text_input(f"Nombre actual: {nombre_linea}", value=nombre_linea, key=f"linea_nombre_{idx}")
        num_estaciones = st.number_input(f"Número de estaciones en '{nuevo_nombre_linea}'", min_value=1, step=1, value=info["num"], key=f"num_estaciones_{idx}")

        estaciones = []
        for j in range(num_estaciones):
            nombre_est = info["nombres"][j] if j < len(info["nombres"]) else ""
            estaciones.append(st.text_input(f"Nombre estación {j+1} de línea '{nuevo_nombre_linea}'", value=nombre_est, key=f"estacion_{idx}_{j}"))

        nuevas_lineas[nuevo_nombre_linea] = {
            "num": num_estaciones,
            "nombres": estaciones
        }

    if st.button("💾 Guardar cambios"):
        # Actualizar líneas y estaciones
        usuarios[usuario]["lineas"] = nuevas_lineas

        # Si se ha introducido nueva clave, validarla
...
elif st.session_state.etapa == "seleccion_linea":
    usuario = st.session_state.usuario_logueado
