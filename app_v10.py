...
if st.session_state.usuario_logueado:
    ...
    if st.button("Agregar línea"):
        if nueva_linea in usuarios[usuario]["lineas"]:
            ...
            usuarios[usuario]["lineas"][nueva_linea] = num_estaciones
            with open(DATA_FILE, "w", encoding="utf-8") as f:
                json.dump(usuarios, f, indent=4)
    ...
    for linea, estaciones in usuarios[usuario]["lineas"].items():
        st.write(f"🔹 {linea}: {estaciones} estaciones")
...
# Crear estructuras para guardar estado por estación
for cee in cees:
...
def eliminar_registro_por_indice(indice):
    ...
    # Construir la clave del estado
    key_estado = f"estado_linea_{cee}-{linea}_Estacion_{estacion}"
...
if st.session_state.etapa == "CEE":
    ...
    estacion_seleccionada = st.selectbox("Selecciona la estación", estaciones, index=0, key="select_estacion")
    if st.button("Continuar", key="btn_estacion"):
        num_estacion = int(estacion_seleccionada.split()[-1])  # Extrae solo el número
        st.session_state.estacion = num_estacion
    ...
    key_estado = f"estado_linea_{st.session_state.cee}-{st.session_state.linea}_Estacion_{st.session_state.estacion}"
    if key_estado not in st.session_state:
        st.session_state[key_estado] = "No iniciada"
        guardar_estados()  # <-- GUARDAR CAMBIOS
    ...
    btn_key_iniciar = f"iniciar_{st.session_state.cee}_{st.session_state.linea}_{st.session_state.estacion}"
    iniciar_disabled = st.session_state[key_estado] == "En producción"
    if st.button("🟢 Iniciar Producción", disabled=iniciar_disabled, key=btn_key_iniciar):
        if st.session_state[key_estado] == "Parada por incidencia":
            ...
            st.session_state.df.to_csv(st.session_state.csv_path, index=False, sep=';', encoding='utf-8-sig')
            # Establecer "En producción" en todas las estaciones
            for cee in cees:
                for linea in lineas_por_cee[cee]:
                    key_linea = f"{cee}-{linea}"
                    num_estaciones = estaciones_por_linea[key_linea]
    ...
    btn_key_parada = f"parada_{st.session_state.cee}_{st.session_state.linea}_{st.session_state.estacion}"
    ...
    btn_key_finalizar = f"finalizar_{st.session_state.cee}_{st.session_state.linea}_{st.session_state.estacion}"
