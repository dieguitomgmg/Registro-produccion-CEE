...
if (
    ...
    # Modo inicial normal (no forzado aquí)
    modo = st.radio("¿Tienes una cuenta?", ["Iniciar sesión", "Registrarse"], key="modo")
    ...
    if st.session_state.get("registro_completado"):
        st.success("✅ Registro exitoso. Por favor, refresca la página para iniciar sesión con tu nuevo usuario.")
        # No borramos el flag todavía, lo borramos solo después de refrescar manual
    ...
    if modo == "Iniciar sesión":
        ...
        if st.button("¿Has olvidado tu clave?"):
        ...
        pregunta_seguridad = st.text_input("🔐 Pregunta de seguridad (ej. ¿Tu comida favorita?)")
        respuesta_seguridad = st.text_input("🔑 Respuesta a la pregunta de seguridad", type="password")
        ...
        productos = []
        num_productos = st.number_input("Número de productos diferentes", min_value=1, step=1, key="num_productos_registro")

        for p in range(int(num_productos)):
            nombre_producto = st.text_input(f"🏷️ Nombre del producto {p+1}", key=f"nombre_producto_{p}")
        ...
        if st.button("Registrarse"):
            if nuevo_usuario in usuarios:
                ...
                st.warning("❌ Las claves no coinciden. Vuelve a escribirlas.")
                ...
                st.success("✅ Registro exitoso. Por favor, refresca la página para iniciar sesión.")

# Limpieza del flag registro_completado solo cuando ya no se muestra el mensaje
# (Opcional: puedes borrarlo si el usuario ya cambió a modo iniciar sesión o similar)
if st.session_state.get("registro_completado") and st.session_state.get("modo") == "Iniciar sesión":
...
if "df" not in st.session_state:
    st.session_state.df = pd.DataFrame(columns=[
        ...
        "Nº operarios", "Nombre operarios", "Turno", "Producto", "Unidades producidas",
...
def eliminar_registro_por_indice(indice):
    ...
    es_ultimo = indice == df.index[-1]  # ✅ Validar si es el último registro ANTES de eliminar

    # Validar si es parada o reanudación y si se puede eliminar
    if estado_actual in ["Parada producción", "Reanudación producción"]:
        if not es_ultimo:
            st.warning("⚠️ Solo se puede eliminar una parada o reanudación si es el último registro.")
            return
        if estado_actual in ["Parada producción", "Reanudación producción"]:
            if not estacion:  # Si la estación está vacía, afecta a toda la línea
                if linea != st.session_state.linea:
                    st.warning(
                        f"⚠️ No se puede eliminar un registro de estado general de otra línea.\n\n"
                        f"Registro seleccionado: {linea} (toda la línea)\n"
                        f"Estás en: {st.session_state.linea} - Estación {st.session_state.estacion}"
                    )
                    return
            else:  # Si tiene estación, afecta solo a esa estación
                if (
                    linea != st.session_state.linea or
                    str(estacion).strip() != str(st.session_state.estacion).strip()
                ):
                    st.warning(
    ...
    # Clave de estado
    if not estacion or str(estacion).strip() == "":  # Afecta a toda la línea
        key_estado = f"estado_linea_{linea}"
        # Limpieza previa de 'Estación' para evitar fallos
        df["Estación"] = df["Estación"].fillna("").astype(str).str.strip()
        filtro_estado = (df["Línea"] == linea) & (df["Estación"] == "")
    else:  # Afecta solo a una estación concreta
        estacion = str(estacion).strip()
        key_estado = f"estado_linea_{linea}_Estacion_{estacion}"
        filtro_estado = (df["Línea"] == linea) & (df["Estación"] == estacion)

    # Eliminar el registro
    df = df.drop(indice).reset_index(drop=True)
    # Limpieza tras eliminar el registro para mantener consistencia
    df["Estación"] = df["Estación"].fillna("").astype(str).str.strip()
    st.session_state.df = df

    # Si era el último y era un estado, recuperar el estado anterior
    if es_ultimo and estado_actual in ["Parada producción", "Reanudación producción"]:
        if not estacion or estacion == "":
            # Actualizar estado general de la línea
            df_filtrado_linea = df[(df["Línea"] == linea) & (df["Estación"] == "")]
            if not df_filtrado_linea.empty:
                ultimo_estado_linea = df_filtrado_linea.iloc[-1]["Estado"]
                if ultimo_estado_linea == "Reanudación producción":
                    st.session_state[key_estado] = "En producción"
                elif ultimo_estado_linea == "Parada producción":
                    st.session_state[key_estado] = "Parada por incidencia"
                else:
                    st.session_state[key_estado] = "En producción"
            else:
                st.session_state[key_estado] = "No iniciada"
                st.info("ℹ️ Se ha eliminado el último registro de línea. No quedan registros en esa línea.")

            # Obtener usuario desde el registro
            usuario = registro["CEE"]
            # Actualizar estados de estaciones de la línea
            estaciones_linea = usuarios[usuario]["lineas"][linea]["nombres"]
            for est in estaciones_linea:
                est = str(est).strip()
                if est == "":  # Ya está el estado general
                    continue
                key_estado_est = f"estado_linea_{linea}_Estacion_{est}"
                df_filtrado_est = df[(df["Línea"] == linea) & (df["Estación"] == est)]
                if not df_filtrado_est.empty:
                    ultimo_estado_est = df_filtrado_est.iloc[-1]["Estado"]
                    if ultimo_estado_est == "Reanudación producción":
                        st.session_state[key_estado_est] = "En producción"
                    elif ultimo_estado_est == "Parada producción":
                        st.session_state[key_estado_est] = "Parada por incidencia"
                    else:
                        st.session_state[key_estado_est] = "En producción"
                else:
                    st.session_state[key_estado_est] = "No iniciada"
        else:
            # Caso estación concreta (igual que antes)
            ...
            if not df_filtrado.empty:
                ...
                st.session_state[key_estado] = "No iniciada"
                st.info("ℹ️ Se ha eliminado el último registro. No quedan registros en esa estación o línea.")

        guardar_estados()
        st.session_state.produccion_iniciada = (
            st.session_state[f"estado_linea_{linea}"] == "En producción"
        )
...
if st.session_state.etapa == "inicio_usuario":
    ...
    st.title(f"📦 Registro de Producción - {st.session_state.usuario_logueado}")

    st.write("Selecciona una opción:")

    col1, col2 = st.columns(2)
    with col1:
        if st.button("📊 Ir a producción"):
            # Flujo actual que lleva a selección de línea
            st.session_state.etapa = "linea"
            st.rerun()
    with col2:
        if st.button("⚙️ Configuración de usuario"):
            st.session_state.etapa = "configurar"
            st.rerun()


elif st.session_state.etapa == "recuperar_clave":
    st.title("🔐 Recuperar clave de acceso")
    ...
    if st.session_state.subetapa_recuperar == "introducir_usuario":
        usuario_recuperar = st.text_input("👤 Introduce tu nombre de usuario (CEE)", key="usuario_recuperar_input")
        ...
        if pregunta:
            ...
            respuesta_introducida = st.text_input("✏️ Tu respuesta", type="password", key="respuesta_recuperar")

            if st.button("📬 Recuperar clave"):
                if respuesta_introducida.strip().lower() == usuarios[usuario_recuperar].get("respuesta", "").lower():
                    st.success(f"✅ Tu clave es: **{usuarios[usuario_recuperar]['clave']}**")
                else:
                    st.error("❌ Respuesta incorrecta. Intenta de nuevo.")
        else:
            st.warning("⚠️ Este usuario no tiene configurada una pregunta de seguridad.")
            st.session_state.subetapa_recuperar = "introducir_usuario"
    ...
    st.title(f"⚙️ Configuración - {st.session_state.usuario_logueado}")

    usuario = st.session_state.usuario_logueado
    datos = usuarios[usuario]["lineas"]

    st.subheader("🔑 Cambiar clave")
    ...
    # Nuevo número total de líneas
    num_lineas_nuevas = st.number_input(
        "Número total de líneas de producción",
        min_value=1,
        step=1,
        value=len(datos),
    ...
    for i in range(int(num_lineas_nuevas)):
        st.subheader(f"✏️ Línea {i+1}")

        nombre_linea_key = f"config_nombre_linea_{i}"
        nombre_linea = st.text_input(f"Nombre de la línea {i+1}", key=nombre_linea_key)

        num_estaciones = st.number_input(
            f"Número de estaciones para línea {i+1}",
            min_value=1,
            step=1,
        ...
        for j in range(int(num_estaciones)):
            ...
            estaciones.append(st.text_input(f"🔤 Nombre estación {j+1} de línea {i+1}", value=nombre_est, key=est_key))
    ...
    st.markdown("---")
    st.subheader("📦 Configurar productos")

    productos_actuales = usuarios[usuario].get("productos", [])
    num_productos = st.number_input(
        "Número total de productos",
        min_value=1,
        step=1,
        value=len(productos_actuales),
        key="config_num_productos"
    )

    nuevos_productos = []

    for i in range(int(num_productos)):
        nombre_default = productos_actuales[i] if i < len(productos_actuales) else ""
        nombre_producto = st.text_input(f"🔤 Nombre del producto {i+1}", value=nombre_default, key=f"config_producto_{i}")
        nuevos_productos.append(nombre_producto)


    if st.button("💾 Guardar cambios", key="boton_guardar_config"):
        usuarios[usuario]["lineas"] = nuevas_lineas
        usuarios[usuario]["productos"] = nuevos_productos

        if nueva_clave.strip() != "":
            if nueva_clave == confirmar_clave:
                usuarios[usuario]["clave"] = nueva_clave
            else:
                st.error("❌ Las claves no coinciden. No se actualizó la contraseña.")
                st.stop()
    ...
    st.subheader(f"Registro en {usuario} / Linea: {linea} / Estación: {estacion}")
        ...
        if st.button("✅ Registrar reanudación"):
            if motivo_reanudacion.strip() == "":
                ...
                if st.session_state.get(clave_linea_parada, False):
                    ...
                    "Estación": "",  # Estación vacía para indicar toda la línea
    ...
    if st.session_state.mostrar_motivo_parada:
        opcion_impacto = st.radio("¿La parada afecta a:", ["Estación", "Línea"], key="opcion_impacto")
    ...
    if st.session_state[key_estado] == "En producción":
        ...
        nombres_operarios = st.text_input(
            "🧑‍🤝‍🧑 Nombres de operarios (opcional)",
            key="nombres_operarios",
            help="Puedes dejarlo vacío si prefieres no especificar nombres."
        )
        turno = st.radio("🕒 Turno", ["Todos", "Mañana", "Tarde", "Noche"])

        productos_usuario = usuarios.get(st.session_state.usuario_logueado, {}).get("productos", [])

        if productos_usuario:
            producto = st.selectbox("📦 Producto", productos_usuario)
        else:
            st.warning("⚠️ Este usuario no tiene productos configurados.")
            producto = None
        ...
        if st.button("✅ Registrar producción"):
            ...
            nueva_fila = {
                ...
                "Nº operarios": num_operarios,
                "Nombre operarios": nombres_operarios,
                "Turno": turno,
                "Producto": producto,
...
if st.session_state.etapa == "linea":
    ...
    if st.session_state.mostrar_edicion:
        ...
        def formatear_registro(row):
            ...
            return f"{row['Fecha']} - {row['Hora']} - {row['CEE']} - {row['Línea']} - {row['Estación']} - {row['Estado']} - {row['Nº operarios']} - {row['Nombre operarios']} - {row['Turno']} - {row['Producto']} - {row['Unidades producidas']} - {row['Unidades defectuosas']} - {row['Observaciones']}"
        ...
        if registro["Estado"] not in ["Reanudación producción", "Parada producción"]:
            nuevo_num_operarios = st.number_input(
                "👥 Número de operarios",
                min_value=1,
                step=1,
                value=registro.get("Número de operarios", 1),
                key=f"num_operarios_{clave_registro}"
            )

            nuevo_nombres_operarios = st.text_input(
                "🧑‍🤝‍🧑 Nombres de operarios (opcional)",
                value=registro.get("Nombres de operarios", ""),
                key=f"nombres_operarios_{clave_registro}"
            )
            turnos_validos = ["Todos", "Mañana", "Tarde", "Noche"]
            ...
            if st.button("💾 Guardar cambios en el registro seleccionado", key=f"guardar_{clave_registro}"):
                ...
                st.session_state.df.at[index_original, "Nº operarios"] = nuevo_num_operarios
                st.session_state.df.at[index_original, "Nombre operarios"] = nuevo_nombres_operarios
