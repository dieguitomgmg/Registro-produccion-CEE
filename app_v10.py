...
if "usuario_logueado" not in st.session_state or not st.session_state.usuario_logueado:
    ...
    # Variable para controlar el modo inicial del radio
    if st.session_state.get("registro_completado"):
        modo_inicial = "Iniciar sesión"
    else:
        modo_inicial = st.session_state.get("modo", "Iniciar sesión")

    # Aquí creas el radio usando el modo_inicial, NO modificas st.session_state.modo directamente
    modo = st.radio("¿Tienes una cuenta?", ["Iniciar sesión", "Registrarse"], index=["Iniciar sesión", "Registrarse"].index(modo_inicial), key="modo")

    # Si detectamos registro completado, limpiamos y recargamos para que el cambio se refleje
    if st.session_state.get("registro_completado"):
